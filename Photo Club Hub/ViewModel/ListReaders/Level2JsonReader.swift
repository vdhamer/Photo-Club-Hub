...
//        }
    ...
    private func mergeLevel2Json(bgContext: NSManagedObjectContext,
        ...
        guard jsonRoot["club"].exists() else { throw MergeError.invalidJsonData("Cannot find club keyword in \(urlComponents.shortName)") }
        let jsonClub: JSON = jsonRoot["club"]
        guard jsonClub["idPlus"].exists() else { throw MergeError.invalidJsonData("Cannot find idPlus keyword in \(urlComponents.shortName)") }
        ...
        else { throw MergeError.mismatchedNameTown("Error: mismatched Name/Town for club \(club.fullNameTown) in \(urlComponents.shortName)") }
