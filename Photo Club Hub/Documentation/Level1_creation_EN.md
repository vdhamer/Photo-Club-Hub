...
Separate instructions exist for adding lists of _club members_ ("Level 2" JSON). Level 2 data can be added at a later time.
...
3. Make a **copy** of the [Max](https://github.com/vdhamer/Photo-Club-Hub/blob/main/JSON/TemplateMax.level1.json) file and give it an appropriate file name:
   ...
   > This means that you have to add any extra fields yourself, for example by copying extra lines from Max.
   > Because JSON tends to be a very strict format, you may introduce errors in the extended file. Typical errors are about commas or mismatched brackets.
   > Don't worry too much about that now: step 8 explains how you can check your file for common problems.
...
Or via a web page generated by us that is reachable from [en/clubs](https://www.fcDeGender.nl/en/clubs).

## What about `level1URLIncludes`?

This chapter is mainly relevant for files with an "unwieldy" number of clubs:
it gives the option of splitting large Level 1 files into smaller files.

So, feel free to initially skip this little chapter about "include" files.
