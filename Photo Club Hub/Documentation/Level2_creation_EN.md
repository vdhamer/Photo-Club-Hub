...
2. Make a **copy** of the [min](https://raw.githubusercontent.com/vdhamer/Photo-Club-Hub/refs/heads/main/JSON/TemplateMin.level2.json) file.
   - Rename the file to some short name (`nickName`) for the club. So if the club is called "F/8 and Be There" you might choose `f8AndBeThere.level2.json`. If your club is already listed in the Photo Club Hub app, you can just reuse the `nickName` from its Level 1 entry (for Dutch clubs, the `clubsNLxx.level1.json` file of its Fotobond afdeling). To make this even simpler, here's what it says for some of the early-adopter clubs:
...
3. **Edit** the file to contain your club’s `fullName`, `town`, `nickName`, and preferably coordinates.
    ...
    -  If your club is already visible in the app, you should simply reuse the `fullName` and `town` from its Level 1 entry. Dutch clubs are listed per Fotobond afdeling in files named `clubsNLxx.level1.json`, for example [clubsNL16.level1.json](https://github.com/vdhamer/Photo-Club-Hub/blob/main/JSON/clubsNL16.level1.json) for Brabant Oost. Again, the early adopters should find all the info they need in the above table.
        - If your club is not listed yet, provide the club's `fullName` and `town`. Together these 2 fields uniquely identify the club: there better not be two clubs called "f8 and Be There" in Amsterdam. But there could be one that happens to have the same `fullName` in Singapore. Be careful to get the spelling right the first time: it gets messy if you need to fix this later.
    - The coordinates here take precedence over those in your club's Level 1 entry. That is deliberate: the club knows its own details best, while Level 1 is often maintained at afdeling level. So this is where your club decides what the map shows, for example a nearby square or landmark instead of where it meets.
