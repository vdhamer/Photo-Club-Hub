...
| Setting the marketing version | development | guideline |
| Bumping the build number | development | guideline |
| Creating `b<number>` build tags | development | guideline |
| Keeping both released and beta Xcode installed | development | policy |
| Getting code only by pulling from origin | release | guideline |
| Archiving for App Store Connect | release | policy |
| Archiving with a released or RC Xcode | release | enforced (by Apple) |
...
therefore stays on a released or RC Xcode whatever the destination, while the development Mac keeps
both installed.
...
2. **Develop** on the development Mac, keeping `ReleaseNotes.md` up to date as work lands.
3. **Push to origin**, then pull on the release Mac — the only channel between the two.
4. **Archive** on the release Mac, with a released or RC Xcode.
5. **Distribute to App Store Connect** from the Organizer.
6. **Keep the `.xcarchive`** on the release Mac, alongside every earlier one.
7. **Release in App Store Connect** to TestFlight, or to TestFlight and then the App Store.
...
one" — is decided at step 7, at the end. The loop therefore may run several times per marketing
version, and a build number is committed to long before anyone knows what will become of it.
