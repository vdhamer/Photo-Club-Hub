...
- **The line-count row is written by a Git pre-commit hook.** `scripts/linecount/pre-commit.sh` runs `countLines.sh` on every commit in this repo and stages `LineCount.csv`, so each commit appends or updates a row for the current day, a day without commits gets no extra row, and the CSV no longer sits modified between running the script by hand and the next commit, which would stamp Debug builds as `-dirty`. It is installed per clone as a symlink, on the primary Mac only, and never blocks a commit; `countLines.command` is gone. A launchd agent, a pre-push hook and GitHub Actions were weighed and rejected ([#850](https://github.com/vdhamer/Photo-Club-Hub/issues/850))

- **Maps stops reverse-geocoding clubs it has already resolved.** 
Every time a club's card was added to the view hierarchy, `FilteredMapsView` asked Apple for a town and country name it was already holding, then wrote the answer back without comparing it. Measured on iOS 26.5: leaving Maps for another tab and returning re-geocoded the same club, because the teardown makes `.onAppear` fire again — one rate-limited request per card that renders, on every visit, indefinitely, and a Core Data write each time. The card now asks only when it holds no address in the user interface language, or when the club's coordinates have moved since that address was derived, which takes the steady state to zero requests. Since re-deriving on every appearance was also what kept those names current, both cases are now handled deliberately rather than as a side effect: a club that relocates is re-geocoded, and a switch to another supported language finds no stored address and derives one. **One deliberate change of behaviour comes with it.** The request never said which language it wanted, and an unqualified reverse-geocode answers in the *device's* language rather than the app's. Measured on a Spanish simulator: the same club and the same coordinates returned "Reusel, Países Bajos" with no language specified and "Reusel, Netherlands" with one. The request now names the language it wants, which makes the languages this line can appear in exactly the languages the app is localized in. That ceiling is intended: a country name arriving in a language the rest of the screen cannot speak is not an improvement on one it can, and the way to serve a German audience is to localize the app for them. Note the asymmetry with `MapsViewRemark` on the same card, which still follows the device's preferred languages — a remark is somebody's translation work, which is worth showing wherever it exists, while a town and country are derived labels that this app can always produce itself. Requires Photo Club Hub Data 3.2.0. Moving the geocoding call itself into the package, where the website generator already queues and caches its requests, is Photo-Club-Hub-Data#38 ([#827](https://github.com/vdhamer/Photo-Club-Hub/issues/827))
...
- MapsView preview no longer starts a Level 1 load of the legacy `root.level1.json`. It renders the sample club it creates, so it is deterministic and offline. Required before this app can move off Data 3.0.1, the last release in which the per-level readers were public ([#825](https://github.com/vdhamer/Photo-Club-Hub/issues/825))
...
- Tested with Xcode 27.0 beta 5 (27A5237l), iOS 27.0 beta 5 (24A5408d) and macOS 26.6.1 (25G76).
  Archived on the release Mac with Xcode 26.6, which is what this build was compiled with.
...
* Updated built-in readme text (including adding recently added featurres).
* Minor bug fixes (especially for iOS 17/18, updated Preferences screenshot).

CODE MAINTENANCE

* Fixed 1 test ("Parse TemplateMin.level2.json").
* Tested with iOS 26.5, MacOS 26.5 and Xcode 26.5 (17F42).
...
* Added "Studio" as newly supported expertise (for Feb 2026).
...
* Fixed bug: OrganizationListView1718 was used instead of OrganizationListView2627 for iOS 26. Oops.
