...
//
// This test PASSES while the installed RocketSim is a confirmed-broken version
// (meaning the workaround is still needed). It FAILS when a new RocketSim ships
// whose version is not yet in confirmedBroken — the signal to run
// scripts/check-rocketsim-bug-1085.sh to determine whether the crash is fixed.
...
struct RocketSimBug1085Tests {
    ...
    func installedRocketSimVersionIsKnownBroken() {
        ...
        #expect(
            confirmedBroken.contains(majorMinor),
            "RocketSim \(version) is not in the known-broken set \(confirmedBroken). "
            + "Run scripts/check-rocketsim-bug-1085.sh to probe whether the #1085 crash "
            + "still reproduces. If it does: add \"\(majorMinor)\" to confirmedBroken. "
            + "If it is fixed: remove the RS_BROKEN workarounds from capture-screenshots.sh "
