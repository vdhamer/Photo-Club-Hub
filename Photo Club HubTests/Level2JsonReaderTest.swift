...
@MainActor @Suite("Tests the Level 2 JSON reader") struct Level2JsonReaderTests {
    ...
    @Test("Parse XampleMin.level2.json") func xampleMinParse() async {
        ...
        await Model.deleteAllCoreDataObjects(context: bgContext) // internally switches to appropriate thread
...
//        #expect(await PhotographerKeyword.count(context: bgContext) == 0)  // A club without PhotographerKeywords
...
//        #expect(organizations.count == 1) // TODO Test doesn't work because of concurrency issue
    ...
    @Test("Parse XampleMax.level2.json") func xampleMaxParse() async {
        ...
        await Model.deleteCoreDataKeywords(context: bgContext) // This test does have Keywords
