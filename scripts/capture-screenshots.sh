...
declare -A TAB_LABEL_en=( [Maps]="Maps"    [Clubs]="Clubs" [People]="People"   [Settings]="Settings" )
declare -A TAB_LABEL_nl=( [Maps]="Kaarten" [Clubs]="Clubs" [People]="Personen" [Settings]="Instellingen" )
...
# Build the screenshot flag array once.
screenshot_flags() {
    local flags=(--udid "${UDID}" --bezel "${BEZEL}" --background "${BACKGROUND}")
    [[ "${DEVICE_SHADOW}" -eq 1 ]] && flags+=(--device-shadow)
    printf '%s\n' "${flags[@]}"
}
...
capture() {
    ...
    mapfile -t _flags < <(screenshot_flags)
    "${RS}" screenshot "${_flags[@]}" > "${out}"
    echo "  saved ${out}"
}
...
for lang in "${LANGUAGES[@]}"; do
    # Indirect lookup of the per-locale label map (bash nameref).
    declare -n LABELS="TAB_LABEL_${lang}"
