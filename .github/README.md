...
(left part of image) or might be a member of one club and former member of another. Such individuals are shown having
multiple portfolios. These portfolios can scroll horizontally.
...
The entities (rounded boxes) are tables and the arrows are relationships in the underlying SQLite database.</p>
...
It could be used to drive a value picker in a data entry tool.
For now, it ensures that each `Organization` belongs to exactly one of the supported `OrganizationTypes`.
And it can be used to extract statistics about how many `Organizations` of each `OrganizationType` are supported.
</details></ul>
