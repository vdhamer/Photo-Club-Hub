...
conceivably store their list of members (`MemberPortfolios`) any way they like.
And it up to the software to convert it to the fixed data model.
Similarly, a club can store their list of `Images` per member (`MemberPortfolio`) any way they like.

So, the module loads membership and portfolio data somewhere on the internet (typically the club’s webserver)
and uses it to initialize the in-app database. But also uses it to update the in-app database (e.g., when
members join or new images need to be added to the portfolio).

For Photo Club Waalre, the membership data is read from a HTML table on a password protected
part of the club’s website. HTML is messy to parse, but means the same table can be viewed directly as part of the website.

The portfolios use a somewhat more robust solution: 
they are read from XML files generated by a Adobe Lightroom Web plug-in called [JuiceBox-Pro](https://www.juicebox.net/). 
