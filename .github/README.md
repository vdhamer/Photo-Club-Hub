...
- **Mandatory** fields
    - `clubs` and `museums` are required to distinguish photo clubs from photo museums. Syntactially either can be omitted, but then you wouldn't have photo clubs or museums in the app. For the app's internal database `clubs` and `museums` determine the `OrganizationType` (`club` or `museum`) of each `Organization` object, which in turn determines which marker type is shown on maps.
